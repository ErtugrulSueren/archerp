...
class Musteri(Document):
    def after_insert(self):
        # 120.001 Ana Hesabını Bul (Yurtiçi Alıcılar)
        # İsmi veya kodu tam eşleşmeli, çünkü artık belirli bir alt grup istiyoruz.
        parent_account = frappe.db.get_value("Hesap", 
            {"hesap_kodu": "120.001", "is_group": 1}, 
            "name"
        )
        
        if not parent_account:
            frappe.msgprint("Uyarı: '120.001' kodlu Ana Hesap Grubu (Yurtiçi Alıcılar) bulunamadı. Lütfen önce bu grubu oluşturun.")
            return
        ...
        # Yeni Hesap Kodu Hesapla (Örn: 120.001.001)
        new_code = ""
        if parent_code:
            ...
            if last_account and last_account[0][0]:
                ...
                if len(parts) > 2 and parts[-1].isdigit(): # 120.001.001 (3 parça)
