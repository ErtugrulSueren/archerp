// --- Satin Alma Teklifi JS ---
frappe.ui.form.on('Satin Alma Teklifi', {
    onload: function (frm) {
    ...
    refresh: function (frm) {
        // 3. Hesaplamaları Tetikle (Görünürlük için)
        if (!frm.doc.__islocal && frm.doc.docstatus === 0) {
            calculate_totals(frm);
        }

        if (frm.doc.docstatus === 1 && frm.doc.status !== "İptal" && frm.doc.status !== "Reddedildi") {
            frm.add_custom_button(__('Sipariş Oluştur'), function () {
                frappe.model.open_mapped_doc({
                    method: "archerp.satin_alma.doctype.satin_alma_teklifi.satin_alma_teklifi.make_satin_alma_siparisi",
                    frm: frm
                })
            }, __('Oluştur'));
        }
    ...
    para_birimi: function (frm) {
        if (frm.doc.para_birimi && frm.doc.firma && frm.doc.para_birimi == frappe.get_doc(":Company", frm.doc.firma).default_currency) {
            frm.set_value('doviz_kuru', 1.0);
        }
    }
});

frappe.ui.form.on('Satin Alma Teklif Kalemi', {
    kalemler_remove: function (frm) {
        calculate_totals(frm);
    },
        ...
        frappe.db.get_value('Urun', row.urun, ['standart_alis_fiyati', 'vergi_sablonu', 'stok_birimi', 'gider_hesabi'])
            .then(r => {
                if (r && r.message) {
                    ...
                    // if (r.message.gider_hesabi) frappe.model.set_value(cdt, cdn, 'gider_hesabi', r.message.gider_hesabi); // Teklif için gerekli mi? Teklif Kalemi'nde gider hesabı var mı? Check.
                }
            });
    },
